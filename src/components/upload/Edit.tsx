...
function Edit(): JSX.Element {
  ...
  const submitForm = async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_BASE_URL}/content/edit?uploadID=${uploadId}`,
        {
          method: "put",
          headers: new Headers({
        ...
        <div className={"col-md-12 form-wrapper"}>
          ...
          {submitSuccess && (
            <div className="alert alert-info" role="alert">
              The upload has been edited successfully!
            </div>
          )}
          <form
            id={"create-upload-form"}
